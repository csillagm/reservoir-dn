...
if load_data == False:
    ...
    [random, fourier, frequencies] = representation.sample_binaries(n_samples, N, save_to_npz=True)
...
# plt.title("Average over %d simulations (sample=%d, N= %d)" %(repeat, n_sample, N))
...
plt.legend(("uniformly sampled in signal space", "uniformly sampled in solution space"), loc="best", frameon=False,
           fontsize=16)
plt.tick_params(axis="both", labelsize=16)
plt.tight_layout()
if save:
    plt.savefig(os.path.join('2b.png'), format='png', dpi=200)
if show:
    plt.show()
plt.close()
...
#set for appropriate layout according to simulation parameters (currently set for n_samples = 3*1e6, N=20)
n_bins = 110
ylimits = [1e-3,1e5]
...
if save:
    plt.savefig(os.path.join('2c.png'), format='png', dpi=200)
if show:
    plt.show()
